...
(function () {
  ...
  function buildKDTree(atlas) {
    const N = atlas.size;
    const vecs = atlas.vecs;
    /* Indices into atlas.vecs (atlas point ids). */
    ...
    function build(lo, hi, depth) {
      ...
      /* Median-of-N partition (sort the slice — N small per split). */
      ...
      return {
        ...
        right: build(m + 1, hi, depth + 1),
      };
    }
    return { root: build(0, N, 0), atlas: atlas };
  }
  function _sqDist6(vecs, a, v) {
    const b = a * KD_DIM;
    const d0 = vecs[b]     - v[0];
    const d1 = vecs[b + 1] - v[1];
    const d2 = vecs[b + 2] - v[2];
    const d3 = vecs[b + 3] - v[3];
    const d4 = vecs[b + 4] - v[4];
    const d5 = vecs[b + 5] - v[5];
  ...
  function selectKDTree(tree, vec) {
    ...
    function descend(node) {
      if (!node) return;
      const d = _sqDist6(vecs, node.idx, vec);
      if (d < bestD) { bestD = d; best = node.idx; }
      const axisDelta = vec[node.axis] - vecs[node.idx * KD_DIM + node.axis];
      const near = axisDelta < 0 ? node.left : node.right;
      const far  = axisDelta < 0 ? node.right : node.left;
      descend(near);
      if (axisDelta * axisDelta < bestD) descend(far);
    }
    descend(tree.root);
    return best;
  }
  function selectAllKDTree(tree, vectors, outIndices) {
    ...
    for (let c = 0; c < count; c++) {
      const b = c * KD_DIM;
      tmp[0] = vectors[b]; tmp[1] = vectors[b+1]; tmp[2] = vectors[b+2];
      tmp[3] = vectors[b+3]; tmp[4] = vectors[b+4]; tmp[5] = vectors[b+5];
      outIndices[c] = selectKDTree(tree, tmp);
    }
    return outIndices;
  }
